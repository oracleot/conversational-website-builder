...
6. **UX & UI Quality**: Validate layout, responsiveness, accessibility, and interaction design (use `frontend-design` skill)
7. **Next.js/React Best Practices**: Validate client/server boundaries, rendering performance, and common pitfalls (use `vercel-react-best-practices` skill)
...
**6e. UX/Performance Checklist (Required)**

During E2E testing, also validate and report:

- **Responsive**: Mobile/tablet/desktop layouts don’t overflow or clip; key actions remain visible
- **Accessibility**: Keyboard navigation works; focus states visible; ARIA labels/roles where needed
- **Loading & empty states**: Skeletons/spinners/empty-state copy are present and helpful
- **Hydration/console noise**: No hydration warnings; minimal console warnings/errors
- **React/Next pitfalls**: Avoid unnecessary client components; no expensive re-renders in common flows; no layout shifts for core UI
```
...
- [Summarize responsive, accessibility, loading-state, and rendering/performance findings]
...
When proposing fixes, prefer solutions aligned with:
- `frontend-design` (clear hierarchy, spacing, responsiveness, accessibility)
- `vercel-react-best-practices` (correct client/server component usage, efficient rendering, stable UI states)
```
